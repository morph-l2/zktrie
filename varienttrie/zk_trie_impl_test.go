...
func TestMerkleTree_Delete_Node(t *testing.T) {
	mt := newTestingMerkle(t, 10)
	...
	hash, err := mt.Root()
	assert.NoError(t, err)
	assert.Equal(t, hash.BigInt().String(), "14304783584722290033806493643108201125865220885133156145327037924652605702242")

	err = mt.DeleteWord(zkt.NewByte32FromBytes([]byte{1}))
	assert.NoError(t, err)

	hash, err = mt.Root()
	assert.NoError(t, err)
	assert.Equal(t, hash.BigInt().String(), "9253345441768473360303705532276164724879644075316812412120726028852769301421")
}

func TestMerkleTree_Path_Hash(t *testing.T) {
	mt := newTestingMerkle(t, 10)
	...
	hash, err := mt.Root()
	assert.NoError(t, err)
	assert.Equal(t, hash.BigInt().String(), "12213141318713358164407947900504257281071497358201079173013367595674828356088")
}
